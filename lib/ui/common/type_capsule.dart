...
/// Капсула «Витрата / Дохід» — одна кнопка, яка показує поточний тип і
/// перемикається тапом (Функціонал п.2.1, рішення 28).
/// Витрата — нейтральна (норма не потребує кольору), дохід — зелений
/// (рішення 50 переглянуло первісний синій).
/// Без вібрації; відгук візуальний за --d-micro.
///
/// Сама по собі нічого не знає про стан вводу: на Екрані 1 її одягає
/// [TypeSwitch], у шторці нової категорії вона тримає локальний вибір
/// (рішення 93).
class TypeCapsule extends StatelessWidget {
  const TypeCapsule({super.key, required this.type, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    final color = isIncome ? AppColors.income : AppColors.textPrimary;
